...
export interface BiometricFortune {
  thienDinh: string;
  taiBach: string;
  phuThe: string;
  tongQuan: string;
...
export const handleScan = async (base64Image: string) => {
  const analysis = await analyzeImage(base64Image);
  const { objectKey: imageKey, region } = await persistScanResult(base64Image);
